...
### 2.2 Intent Router — LLM-first, rule-based fallback

Default router là **`LLMIntentRouter`** ([core/llm_router.py](src/fintextsql/core/llm_router.py)) — gọi LLM 1 lần với prompt phân loại + extract ticker, parse JSON response. Khi LLM down/timeout/JSON malformed, tự động fallback sang **`RuleBasedRouter`** ([core/intent.py](src/fintextsql/core/intent.py)) để hệ thống không bao giờ "treo".

**5 intent** (giảm từ 7 sau refactor 2026-05):

```
1. text_to_sql    — truy vấn data thật (giá, volume, return, MA, drawdown, P/E của mã cụ thể)
2. visualization  — yêu cầu vẽ chart (line/bar/scatter), gọi text_to_sql nội bộ
3. web_search     — Tavily: news + company facts (CEO, founder, website, trụ sở)
4. ingestion      — sync/cập nhật dữ liệu từ yfinance
5. general        — LLM conversational: kiến thức tài chính (P/E là gì), chào hỏi, định nghĩa
```

**LLM router prompt** chốt rõ ranh giới gây nhầm lẫn:
- `P/E của AAPL` → text_to_sql (cần data thật)
- `P/E là gì` → general (kiến thức phổ thông)
- `CEO của Apple` → web_search (data ngoài DB)
- `Apple làm ăn thế nào gần đây` → web_search (cần tin tức cập nhật)

LLM cũng extract ticker (kể cả khi user gõ "apple", "appl"), bù cho `extract_tickers()` regex còn yếu với tên công ty.

**Latency overhead**: +200-500ms cho 1 LLM call thêm. Acceptable vì giảm false-routing đáng kể so với rule-based.

**Rule-based fallback** (`RuleBasedRouter` trong `core/intent.py`) vẫn dùng word-boundary match (`(?<!\w)needle(?!\w)`) tránh false positive như "cái đó thì..." (sau strip dấu → "cai do thi...") match nhầm `"do thi"` (đồ thị).
...
"So sánh AAPL và MSFT và vẽ chart và có tin gì không"
  ...
  → Task 3: web_search (tin tức)
```

Planner nhận `RouteDecision` từ LLM router (truyền qua param `decision=` để khỏi gọi router 2 lần). Tự suy luận `metric`, `time_window`, `grouping`, `comparison` từ câu hỏi; nếu là follow-up (`"cùng khoảng thời gian đó"`) thì reuse từ `SESSION_STATE`.
...
| `web_search` | `paths/web_search/service.py` | **Tavily search** + LLM tóm tắt cho cả news lẫn company facts |
| `ingestion` | `ingestion/yfinance_service.py` | Upsert prices + fundamentals từ yfinance (news ingestion đã bỏ) |
| `general` | `paths/general/service.py` | **LLM conversational** với FinTextSQL persona; fallback static khi LLM down |

**Refactor 2026-05**: gộp `news` + `company_info` → `web_search` (cùng pattern Tavily + LLM summary), xóa `simple_finance` (text_to_sql đủ sức trả lời "giá hiện tại"), nâng `general` từ static blurb lên LLM-powered. 7 paths → 5 paths.
...
### 3.1 ~~Deterministic SQL Builders~~ — RETIRED (2026-05)

Trước đây có ~32 hand-written SQL builder hard-code cho các pattern phổ biến (year_high_close, top_volume, growth_stability_ranking...). Mỗi builder gồm cặp `_asks_for_X()` detector + `_X_sql()` generator. Khi câu hỏi match, pipeline bỏ qua LLM hoàn toàn → nhanh và chính xác.

**Đã xoá khỏi pipeline** trong commit `a5e2228`:
- Mọi câu hỏi giờ đi qua đầy đủ Load Schema → Knowledge → Schema Selector → Planner → SQL Generator → Candidate Generator x3 → Candidate Selector → Execute SQL.
- Lý do: kiến trúc thuần LLM (theo image `docs/Text2sql_report.docx`), dễ maintain hơn, không bị mismatch giữa builder detector và LLM understanding.
- Trade-off: latency tăng (best case 2 LLM call vs 0), accuracy giảm với pattern phức tạp khi LLM yếu.

Code 30+ builder functions vẫn tồn tại trong [text2sql/service.py](src/fintextsql/text2sql/service.py) nhưng **unreachable**. Giữ lại để có thể bật lại làm fast-path khi benchmark đòi hỏi.

Các **deterministic explainer** (`_deterministic_*_explanation`) vẫn active — chúng format rows tiếng Việt sau khi LLM sinh SQL, KHÔNG sinh SQL.
...
- **News ingestion đã bỏ** sau refactor 2026-05 (web_search dùng Tavily live tại query time, không cần cache RSS vào DB).

### 6.2 Tavily Web Search — `paths/web_search/service.py`

**Unified path** cho cả news (tin tức) lẫn company facts (CEO, founder, headquarters, website, ngành nghề). Service tự detect "flavour" dựa trên keyword:
- `news` flavour: prefix query "Latest news:", chuyển sang `topic=news, time_range=month` để ưu tiên tin mới.
- `fact` flavour: query thuần, `topic=general` để search rộng.
...
LLM tóm tắt snippet với 2 prompt khác nhau theo flavour. Với fact lookup, prompt **time-aware** so sánh ngày sự kiện với hôm nay để xác định đúng người đương nhiệm (tránh hallucination "X đã thành CEO" cho sự kiện tương lai).

Fallback graceful khi `TAVILY_API_KEY` rỗng hoặc Tavily/LLM timeout — hiển thị raw snippet để user tự verify.
...
3. **`text2sql/service.py` 3355 dòng** với 30+ builder function unreachable → nên xoá dead code hoặc tách thành module riêng `deterministic_sql/` (giữ làm fast-path optional).
...
6. **LLM router latency** thêm 200-500ms/request. Có thể cache decision theo question hash khi cùng câu hỏi được hỏi lại trong session.
7. **Frontend TypeScript intent union** chưa update theo 5-intent mới — `news`, `company_info`, `simple_finance` cần xoá khỏi type literal.
8. **Bảng `news_articles`** vẫn tồn tại nhưng không còn được populate. Có thể drop trong migration tiếp theo.
